...
class ChartGenerator:
    ...
    def plot_daily_trend(self, data: List[Dict], title: str = "每日营收趋势") -> str:
        """
        每日营收趋势图（多日数据）
        """
        if not data:
        ...
        # 营收柱状图
        bars = ax.bar(df['date'], df['revenue'], color=self.COLORS['primary'], alpha=0.8, label='营收')

        # 营收折线
        ax2 = ax.twinx()
        ax2.plot(df['date'], df['orders'], color=self.COLORS['accent'], marker='o',
                 linewidth=2, markersize=8, label='订单数')
    ...
    def plot_comparison(self, data: Dict, title: str = "同比/环比对比") -> str:
        """
        同比/环比对比图
        """
        ...
        previous = comp.get('previous_period', {})

        if not current or not previous:
            return None

        metrics = ['total_revenue', 'total_orders']
        metric_labels = ['总营收', '订单数']
        metric_formats = ['¥{:.0f}', '{}']

        curr_values = [current.get('total_revenue', 0), current.get('total_orders', 0)]
        prev_values = [previous.get('total_revenue', 0), previous.get('total_orders', 0)]
        changes = [comp.get('revenue_change_pct', 0), comp.get('order_change_pct', 0)]

        fig, axes = plt.subplots(1, 2, figsize=(12, 5))

        for i, (ax, metric, label, fmt, curr, prev, change) in enumerate(
                zip(axes, metrics, metric_labels, metric_formats, curr_values, prev_values, changes)):

            x = [0, 1]
            values = [prev, curr]
            colors = [self.COLORS['gray'], self.COLORS['primary']]
            ...
            # 添加变化率
            change_color = self.COLORS['accent'] if change >= 0 else self.COLORS['danger']
            change_symbol = '↑' if change >= 0 else '↓'
            ax.text(1, curr * 0.5, f'{change_symbol} {abs(change)}%',
                   ha='center', va='center', fontsize=14, fontweight='bold',
                   color=change_color)

            ax.set_xticks(x)
            ax.set_xticklabels(['上期', '本期'])
    ...
    def plot_category_trend(self, data: List[Dict], title: str = "分类每日趋势") -> str:
        """分类每日趋势图"""
        if not data:
            return None
        ...
        color_map = {
            '刺身': self.COLORS['primary'],
            '寿司': self.COLORS['secondary'],
            '卷类': self.COLORS['accent'],
            '烧物': self.COLORS['warning'],
            '炸物': self.COLORS['danger'],
        }

        for cat in categories:
            color = color_map.get(cat, self.COLORS['gray'])
            ax.plot(df['date'], df[cat], marker='o', linewidth=2, label=cat, color=color)
    ...
    def plot_table_efficiency(self, data: Dict, title: str = "桌台效率分析") -> str:
        ...
        翻台率: {data.get('estimated_turnover_rate', 0)}
        平均桌台营收: ¥{data.get('avg_revenue_per_table', 0)}
        使用桌台数: {data.get('unique_tables_used', 0)}
        """
    ...
    def generate_all_charts(self, report: Dict) -> Dict[str, str]:
        ...
        if report.get('daily_trend'):
            charts['daily_trend'] = self.plot_daily_trend(report['daily_trend'])

        # 同比/环比对比
        if report.get('comparison'):
            charts['comparison'] = self.plot_comparison(report)
        ...
        # 滞销菜品
        if report.get('bottom_dishes'):
            charts['bottom_dishes'] = self.plot_top_dishes(
                report['bottom_dishes'], 'quantity', '滞销菜品预警'
            )
        ...
        # 分类趋势
        if report.get('category_trend'):
            charts['category_trend'] = self.plot_category_trend(report['category_trend'])
